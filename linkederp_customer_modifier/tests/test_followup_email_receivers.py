...
class TestFollowupEmailReceivers(common.TransactionCase):
    ...
    def test_report_recipients_override(self):
        self.partner.followup_email_receivers = 'custom1@test.com, custom2@test.com'
        recipients = self.env['account.followup.report']._get_email_recipients({'partner_id': self.partner.id})
        self.assertEqual(len(recipients), 2)
        self.assertEqual(set(recipients.mapped('email')), {'custom1@test.com', 'custom2@test.com'})

    def test_report_recipients_fallback_to_billing_contact(self):
        recipients = self.env['account.followup.report']._get_email_recipients({'partner_id': self.partner.id})
        self.assertEqual(recipients, self.partner._get_all_followup_contacts())
