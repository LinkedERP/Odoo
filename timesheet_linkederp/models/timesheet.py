...
class AccountAnalyticLine(models.Model):
    ...
    available_sale_line_domain = fields.Char(
        compute='_compute_available_sale_line_domain',store=False
    )

    @api.depends('project_id','task_id','helpdesk_ticket_id','date','name')
    def _compute_available_sale_line_domain(self):
        for rec in self:
            if rec.helpdesk_ticket_id:
                rec.available_sale_line_domain = json.dumps([
                    ('id', 'in', rec.helpdesk_ticket_id.sale_order_id.order_line.ids)
                ])
            elif rec.task_id:
                rec.available_sale_line_domain = json.dumps([
                    ('id', 'in', rec.task_id.sale_order_id.order_line.ids)
                ])
            else:
                rec.available_sale_line_domain = json.dumps([])
    ...
    @api.depends('helpdesk_ticket_id.sale_line_id','employee_id', 'task_id', 'helpdesk_ticket_id')
    ...
    so_line = fields.Many2one(
        compute="_compute_so_line", store=True, readonly=False,
        domain="available_sale_line_domain", falsy_value_label="Non-billable",
